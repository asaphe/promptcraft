...
5. **Rules must capture the general principle, not just one instance** — When a rule is derived from a specific incident or correction, check whether it would also prevent analogous mistakes. If a rule says "don't use X" but the real principle is "only A, B, C support feature Y", teach the principle. Narrow rules get bypassed by the next variant of the same mistake. Both rule authors and rule reviewers should check for this.
