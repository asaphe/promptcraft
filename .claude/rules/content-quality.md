...
- **Keep examples and scaffolding in sync** — When updating a pattern in `examples/`, check if the same pattern exists in `claude/scaffolding/` and update both. When adding a new hook to `examples/hooks/`, consider whether `claude/scaffolding/` should reference it.
