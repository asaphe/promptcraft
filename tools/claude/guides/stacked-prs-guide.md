...
**Known gap: [`pr-create-guard`](../examples/hooks/pr-create-guard/) does not cover `gh stack submit`.** It matches `gh pr create` only, so its dirty-tree and unpushed-commit checks are silently skipped for a stack — verified by running the shipped hook, which exits 0 on `gh stack submit` while blocking `gh pr create` in the same tree. Submitting a stack with uncommitted changes therefore produces layers missing the work you meant to include, with no warning. If you adopt stacks, extend that hook's matcher before relying on it.
