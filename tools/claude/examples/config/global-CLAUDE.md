...
**Merge a PR only when the user's current message asks for it (opinionated).** "Merge #17" names the PR and authorizes that one turn; an earlier "yes", an approved plan, or a PR list carried over from a previous turn does not. Merge only the PRs the message names, with CI green and the branch up to date read fresh, and never with `--admin`. If mapping the words to specific PRs takes interpretation ("the ones above", a range), confirm the exact list before the first merge. [`merge-grant`](../hooks/merge-grant/) with [`destructive-guard`](../hooks/destructive-guard/) enforces the turn: a merge hard-blocks unless the user's prompt this turn asked for one, and even then raises a permission prompt. Which PRs, and whether CI is green, stay the agent's job. A stricter setup leaves the grant hook out, and every merge stays a hard block. In every turn without that request, hand the merge over as a **clickable PR link**, never as a `gh pr merge` command the user will not run — and carry three things with it: the URL, CI state queried fresh rather than recalled from an earlier turn, and the single item blocking merge, or a plain "nothing does". Mergeable is not the same as should-merge-now: where an operational prerequisite must land first (an apply, a migration, a dependency's release), that line leads the handover, because a green-CI verdict reads as authorization and the forge will happily report a premature merge as mergeable.
