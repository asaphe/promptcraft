...
# Flag-permissive pattern: catches `gh --hostname X api ...`, etc.
if echo "$CMD" | grep -qE 'gh[[:space:]]([^|;&]* )?api +repos/.+/pulls/[0-9]+/reviews' && ! echo "$CMD" | grep -qE '\-X +(GET|DELETE)'; then
...
# PR body comment
if echo "$CMD" | grep -qE 'gh[[:space:]]([^|;&]* )?pr +comment '; then
...
if echo "$CMD" | grep -qE 'gh[[:space:]]([^|;&]* )?api +repos/.+/pulls/[0-9]+/comments ' && ! echo "$CMD" | grep -qE '\-X +(GET|DELETE)'; then
...
if echo "$CMD" | grep -qE 'gh[[:space:]]([^|;&]* )?api +repos/.+/pulls/comments/[0-9]+ +-X +PATCH'; then
