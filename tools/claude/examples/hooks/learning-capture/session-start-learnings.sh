...
if [[ -f "$PENDING_FILE" && -s "$PENDING_FILE" ]]; then
  CANDIDATE_COUNT=$(grep -c '^## \(Session\|Pre-compaction\)' "$PENDING_FILE" 2>/dev/null || echo "0")
  if [[ "$CANDIDATE_COUNT" -gt 0 ]]; then
    ...
    OUTPUT+=" After processing, clear the file."
    OUTPUT+=" Classify: team-wide → .claude/rules/, agent-specific → .claude/agents/\${agent}.md, personal → auto memory."
  fi
fi
