...
- **`destructive-guard/`** — Two-tier PreToolUse hook that hard-blocks irreversible operations (AWS data destruction, any push to main — force included, PR close, all three merge forms unless the user asked this turn, bulk branch deletion) and turns risky-but-approvable ones (terraform destroy, force-push to a feature branch, `git reset --hard`, `gh issue` mutations) into explicit approval prompts. Worktree-aware push detection.
- **`merge-grant/`** — UserPromptSubmit hook that arms a one-turn merge grant when the user's prompt asks for a merge, so `destructive-guard` turns that turn's merges into a permission prompt instead of a hard block.
