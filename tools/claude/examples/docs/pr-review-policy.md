...
- **Route reviews by file scope** — Always determine what changed before choosing a reviewer. Never do ad-hoc reviews in the main context. Spawn all applicable reviewers in parallel for mixed PRs. Tell every agent: "Don't assume — check the codebase and verify every finding before reporting it."
  ...
  | --- | --- |
  | `.github/`, `**/Dockerfile*`, `**/*.sh` (inside `.github/`), `**/*.tf`, `**/values*.yaml` | Spawn `devops-reviewer` agent |
  | `**/*.sh` outside `.github/` (terraform / dev / build / ops utilities) | Spawn `bash-reviewer` agent |
  | `python/**` (standalone tools, not application services) | Spawn `python-reviewer` agent |
  | ClickHouse code (handlers, SQL, migrations, dbt models, CH Terraform) | Spawn `clickhouse-reviewer` agent |
  | Datadog config-as-code (dashboards, monitors, on-calls, indexes) | Spawn `datadog-reviewer` agent |
  | `.claude/` | Spawn `agent-config-reviewer` agent |
  | Application code (Python, TypeScript, Go, Java services) | Spawn `general-reviewer` agent |
  | Mixed | Spawn all applicable reviewers in parallel |

- **Always include `security-reviewer`** — Spawn it on every PR regardless of file types, passing the full file list. Security concerns are cross-cutting (supply chain, GHA injection, OIDC trust, infra hardening, container security, application injection / auth gaps) and not limited to specific file categories.
...
- **Report all review findings at appropriate severity** — When a review identifies multiple issues, report all of them. Don't reduce findings to a single item or silently drop lower-severity ones. Every issue is worth flagging — use severity labels (BLOCKING, ISSUE, SUGGESTION) to distinguish impact, but never omit findings.

- **Verify test plan items before AND after creating a PR — no empty checkboxes** — If the PR body includes a test plan with checkboxes: (1) verify each item before submitting (run tests, check syntax, validate config, wait for workflows), (2) check them off in the body with evidence (links to passing runs, verification commands used), (3) after creating the PR, wait for any auto-triggered workflows to complete and update the checkboxes with results. Never leave unchecked boxes — if a check can't be verified yet, say so explicitly rather than leaving it blank. A PR with unchecked test plan items signals unfinished work.

- **Use `/pr-review` to review, `/pr-resolver` to fix** — When a user says "review this PR", invoke `/pr-review`. When they say "fix the comments" or "address the review", invoke `/pr-resolver`. `/pr-check` covers CI status checks and can also triage / fix / resolve review comments — it's the all-in-one command. `/pr-resolver` is the dedicated tool when the user only wants to address review comments without CI checks.
...
- **One PR should be enough — audit comprehensively the first time** — Before opening a PR that touches a manifest, config, or list of items, audit the entire scope in one pass. A follow-up PR to fix what the first PR missed signals incomplete work.
