...
if echo "$CMD" | grep -qE 'gh\s+api\s+repos/.+/pulls/[0-9]+/reviews' && ! echo "$CMD" | grep -qE '\-X\s+(GET|DELETE)'; then
...
# PR body comment
if echo "$CMD" | grep -qE 'gh\s+pr\s+comment\s'; then
...
if echo "$CMD" | grep -qE 'gh\s+api\s+repos/.+/pulls/[0-9]+/comments\s' && ! echo "$CMD" | grep -qE '\-X\s+(GET|DELETE)'; then
...
if echo "$CMD" | grep -qE 'gh\s+api\s+repos/.+/pulls/comments/[0-9]+\s+-X\s+PATCH'; then
